...
window.onload = function () {
	...
	imageCompressionOptions = document.getElementById("imageCompressionOptions");
	...
	submitBtn.onclick = function () {
		...
		if (extension == "jpg" || extension == "jpeg" || extension == "png") {
			imageCompressionOptions.style.display = "block";
		} else {
			imageCompressionOptions.style.display = "none";
		}
	...
	encodeBtn.onclick = function () {
		...
		fileReader.onload = function (fileLoadedEvent) {
			...
			let extension = uploadedFile.name.split('.').pop().toLowerCase();
			if (extension === "txt") {
				...
				compressImage(text, uploadedFile.type, quality, uploadedFile.name);
			}
		}
	...
	decodeBtn.onclick = function () {
		...
		fileReader.onload = function (fileLoadedEvent) {
			...
			if (extension === "txt") {
				...
				alert("Image decompression not supported yet.");
			}
		}
...
function compressImage(dataUrl, mimeType, quality, fileName) {
	let img = new Image();
	img.src = dataUrl;
	img.onload = function() {
		...
		myDownloadFile(fileName.split('.')[0] + "_compressed."
